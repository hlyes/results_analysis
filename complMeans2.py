...
#Get vaues
#Plot them
res = pd.DataFrame(columns=["Algorithm","Population","BadDevices","NetSize","MaxPar","Chunk count","FastC","Completion time(s)"])
cpt = 0
                ...
                for bp in df5['BadDevices'].unique(): #Population
                    ...
                    for fc in df6['FastC'].unique():
                        ...
                        for al in df7['Algorithm'].unique():#BadRepartition
                            ...
                            del df8['Experiment']
                            m = float(df8.mean())
                            res.loc[cpt]=[al,p,bp,ns,mp,cs,fc,m]
                            cpt+=1
        ...
        for fc in df2['FastC'].unique():
            ...
            for mp in df3['MaxPar'].unique():
                ...
                for p in df4['Population'].unique():
                    ...
                    print out
                    print folder+os.sep+"out"+os.sep+'ComplGoodFixed-'+str(ns)+'-'+str(cs)+'-'+str(mp)+'-'+str(fc)+'-'+str(int(p))+'-byBadDevices.csv'
                    out.to_csv(folder+os.sep+"out"+os.sep+'ComplGoodFixed-'+str(ns)+'-'+str(cs)+'-'+str(mp)+'-'+str(fc)+'-'+str(int(p))+'-byBadDevices.csv',sep=",")
                    Plot.plot_lines(out,folder+os.sep+"out"+os.sep+'ComplGoodFixed-'+str(ns)+'-'+str(cs)+'-'+str(mp)+'-'+str(fc)+'-'+str(int(p))+'-byBadDevices.eps',{'xaxis_label':'Bad devices (percent)'})
                ...
                for p in df4['BadDevices'].unique():
                    ...
                    out = pd.concat(columns,axis=1)
                    print out 
                    out.to_csv(folder+os.sep+"out"+os.sep+'ComplBadFixed-'+str(ns)+'-'+str(cs)+'-'+str(mp)+'-'+str(fc)+'-'+str(int(p))+'-byGoodDevices.csv',sep=",")
                    Plot.plot_lines(out,folder+os.sep+"out"+os.sep+'ComplBadFixed-'+str(ns)+'-'+str(cs)+'-'+str(mp)+'-'+str(fc)+'-'+str(int(p))+'-byGoodDevices.eps',{'xaxis_label':'Fast devices (percent)'})
...
for ns in res["NetSize"].unique():
    ...
    for cs in df["Chunk count"].unique():
        df2 = df[df['Chunk count']==cs]
        del df2['Chunk count']
        for fc in df2['Population'].unique():
            df3 = df2[df2['Population'] == fc]
            del df3['Population']
            #print df3
            for mp in df3['MaxPar'].unique():
                df4 = df3[df3['MaxPar'] == mp]
                ...
                for p in df4['BadDevices'].unique():
                    ...
                    out = pd.concat(columns,axis=1)
                    print out
                    print folder+os.sep+"out"+os.sep+'ComplGoodBadFixed-'+str(ns)+'-'+str(cs)+'-'+str(mp)+'-'+str(fc)+'-'+str(int(p))+'-byFastC.csv'
                    out.to_csv(folder+os.sep+"out"+os.sep+'ComplGoodBadFixed-'+str(ns)+'-'+str(cs)+'-'+str(mp)+'-'+str(fc)+'-'+str(int(p))+'-byFastC.csv',sep=",")
                    Plot.plot_lines(out,folder+os.sep+"out"+os.sep+'ComplGoodBadFixed-'+str(ns)+'-'+str(cs)+'-'+str(mp)+'-'+str(fc)+'-'+str(int(p))+'-byFastC.eps',{'xaxis_label':'FastC (percent)'})
